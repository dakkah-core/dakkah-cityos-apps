...
export function DeliveryMap({ pickup, dropoff, driverLat, driverLng, eta, distance }: DeliveryMapProps) {
  const centerLat = driverLat || (pickup.lat + dropoff.lat) / 2;
  const centerLng = driverLng || (pickup.lng + dropoff.lng) / 2;

  const staticMapUrl = `https://maps.googleapis.com/maps/api/staticmap?size=600x300&scale=2&maptype=roadmap` +
    `&markers=color:blue|label:P|${pickup.lat},${pickup.lng}` +
    `&markers=color:red|label:D|${dropoff.lat},${dropoff.lng}` +
    (driverLat && driverLng ? `&markers=color:green|label:Y|${driverLat},${driverLng}` : "") +
    `&path=color:0x3182ceFF|weight:3|${pickup.lat},${pickup.lng}|${dropoff.lat},${dropoff.lng}`;
...
const styles = StyleSheet.create({
  ...
  mapPlaceholder: { backgroundColor: "#e8f4f8", borderRadius: 14, padding: 16, borderWidth: 1, borderColor: "#bae6fd" },
  routeVisual: { gap: 4 },
  ...
  pointAddr: { flex: 1, fontSize: 11, color: COLORS.textSecondary },
